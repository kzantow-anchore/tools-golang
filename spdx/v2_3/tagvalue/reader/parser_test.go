...
func TestParserCanParseTagValues(t *testing.T) {
	...
	if doc.SPDXVersion != spdx.Version {
		t.Errorf("expected SPDXVersion to be %s, got %v", spdx.Version, doc.SPDXVersion)
	}
	if doc.DataLicense != spdx.DataLicense {
		t.Errorf("expected DataLicense to be %s, got %v", spdx.DataLicense, doc.DataLicense)
	}
...
func TestParserStartFailsToParseIfInInvalidState(t *testing.T) {
	...
	err := parser.parsePairFromStart("SPDXVersion", spdx.Version)
	if err == nil {
