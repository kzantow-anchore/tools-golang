...
func TestParser2_3CanParseTagValues(t *testing.T) {
	var tvPairs []tv.TagValuePair

	// create some pairs
	tvPair1 := tv.TagValuePair{Tag: "SPDXVersion", Value: "SPDX-2.3"}
	tvPairs = append(tvPairs, tvPair1)
	tvPair2 := tv.TagValuePair{Tag: "DataLicense", Value: "CC0-1.0"}
	tvPairs = append(tvPairs, tvPair2)
	tvPair3 := tv.TagValuePair{Tag: "SPDXID", Value: "SPDXRef-DOCUMENT"}
...
func TestParser2_3HasDocumentAfterCallToParseFirstTag(t *testing.T) {
	parser := tvParser2_3{}
	err := parser.parsePair("SPDXVersion", "SPDX-2.3")
	if err != nil {
...
func TestParser2_3StartFailsToParseIfInInvalidState(t *testing.T) {
	...
	err := parser.parsePairFromStart("SPDXVersion", "SPDX-2.3")
	if err == nil {
